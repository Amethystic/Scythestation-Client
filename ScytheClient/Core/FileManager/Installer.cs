...
namespace ScytheStation.Core.FileManager
{
    internal class Installer
    {
        ...
        public static void Init()
        {
            ...
            if (!File.Exists($"{Directories.Folder}\\Dependencies\\EacMelon.exe")) { try { var wc = new WebClient(); wc.DownloadFile("https://thats.gg/download/vrchat/EacMelon.exe", $"{Directories.Folder}\\Dependencies\\EacMelon.exe"); FCOUNT2++; } catch { MelonLogger.Error($"[BL] Failed to download a ScytheStation required file"); } }
            if (!File.Exists($"{Directories.GDir}\\Mods\\ScytheStation.dll")) { try { var wc = new WebClient(); wc.DownloadFile("https://scythe.clan.su/ScytheStation.dll", $"{Directories.GDir}\\Mods\\ScytheStation.dll"); FCOUNT2++; } catch { MelonLogger.Error($"[BL] Failed to download a ScytheStation required file"); } } MelonLogger.Msg($"[LOADER] Recieved {FCOUNT2} Required Files");
        }
    }
}
